...
def main():
    ...
    # Since the bubble is mounted on the moving platform, increase this 
    # (e.g., from 1.5 to 3.5 or 5.0) to ignore pump vibrations.
    DRAG_THRESHOLD = 4.0

    # Variables for stability/debounce filtering
    consecutive_count = 0
    REQUIRED_CONSECUTIVE_FRAMES = 3  # Must detect same state for 3 frames before triggering
    candidate_status = "State: Pure Push / No Drag"
    ...
    try:
        last_sent_time = 0
        COMMAND_INTERVAL = 0.5  # Send a command at most every 0.5 seconds
            ...
            if not calibrate:
                ...
                if len(all_marker_data) > 0:
                    ...
                    if magnitude < DRAG_THRESHOLD:
                        raw_drag_status = "State: Pure Push / No Drag"
                    else:
                        ...
                        if 22.5 <= angle < 67.5:
                            raw_drag_status = f"Drag: DOWN-RIGHT"
                        elif 67.5 <= angle < 112.5:
                            raw_drag_status = f"Drag: DOWN"
                        elif 112.5 <= angle < 157.5:
                            raw_drag_status = f"Drag: DOWN-LEFT"
                        elif 157.5 <= angle < 202.5:
                            raw_drag_status = f"Drag: LEFT"
                        elif 202.5 <= angle < 247.5:
                            raw_drag_status = f"Drag: UP-LEFT"
                        elif 247.5 <= angle < 292.5:
                            raw_drag_status = f"Drag: UP"
                        elif 292.5 <= angle < 337.5:
                            raw_drag_status = f"Drag: UP-RIGHT"
                        else:
                            raw_drag_status = f"Drag: RIGHT"
                
                # --- STABILITY / DEBOUNCE FILTER ---
                if raw_drag_status == candidate_status:
                    consecutive_count += 1
                else:
                    candidate_status = raw_drag_status
                    consecutive_count = 1

                # Only update official drag_status if it stays stable across multiple frames
                if consecutive_count >= REQUIRED_CONSECUTIVE_FRAMES:
                    drag_status = candidate_status
                else:
                    drag_status = drag_status if 'drag_status' in locals() else "State: Pure Push / No Drag"
