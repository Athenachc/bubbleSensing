...
# 5. Plot Colorful Yet B&W-Printable Aggregate Graph with Matched Font Sizes
fig, ax = plt.subplots(figsize=(8, 5))

# Plot mean Ground Truth (Solid Black) and Calculated (Dashed Dodgerblue)
ax.plot(common_time, mean_sensor, color='black', linewidth=2.2, label='Mean Measured Force')
# ax.plot(common_time, mean_calc, color='dodgerblue', linewidth=2, linestyle='--', label=f'Mean Calculated (radio_N = {best_radio_N:.5f})')
ax.plot(common_time, mean_calc, color='dodgerblue', linewidth=2, linestyle='--', label=f'Mean Detected Force')

# Add subtle standard deviation envelopes (distinct contrast shades for B&W printing)
...
# Highlight the discrepancy gap in vibrant orange
...
# Legend font size matched to axis label font size (fontsize=10)
